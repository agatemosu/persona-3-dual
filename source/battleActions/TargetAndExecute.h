    ...
    bool update(u32* keys, Skill* skill, PartyMember* user, std::vector<BattleParticipant*>* targets)
    {
        ...
        if (*keys & KEY_A)
        {
            ...
            if (skill->skillType == SkillType::Attack)
            {
                ...
                u32 accuracy =
                    attackSkill->calculateHitratePlayer(&static_cast<PartyMember*>(user)->curPersona->battleStats,
                ...
                u32 damage =
                    attackSkill->calculateDamagePlayer(&static_cast<PartyMember*>(user)->curPersona->battleStats,
                                                       &static_cast<PartyMember*>(target)->curPersona->battleStats,
                                                       &user->lv,
                                                       &target->lv);
