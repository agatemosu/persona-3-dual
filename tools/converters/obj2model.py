import struct, argparse, json, os, re
...
def build_display_list_for_faces(faces, vertices, texcoords,
                                  tex_width, tex_height,
                                  vertex_color, scale, offset,
                                  blender_source):
...
# Main converter
def convert_model_json(input_file, output_file,
                       vertex_color=None, scale=None, target_size=4.0,
                       center=True, blender_source=False,
                       rgba_all=False, rgba_list=None):
    ...
    with open(bin_out, 'wb') as out:
        ...
        for tex_key, _, w, h, is_rgba in tex_info:
            ...
            out.write(struct.pack('<64s H H B 3x', name_bytes, w, h, 1 if is_rgba else 0))
        ...
        for node_idx, node in enumerate(data['nodes']):
            ...
            for slot, faces in slot_faces.items():
                ...
                words = build_display_list_for_faces(
                    faces, v, vt, tw, th,
                    vertex_color, scale, offset, blender_source
                )
    ...
    with open(header_out, 'w') as h:
        ...
        h.write(f"// Auto-generated by obj2model.py - DO NOT EDIT.\n")
        h.write(f"// Source: {os.path.basename(input_file)}\n")
        h.write(f"// Scale:  {scale:.6f}   Centred: {center}\n\n")
        h.write(f'#include "controllers/AnimationController.h"\n\n')
        ...
        h.write(f"// Call after AnimationController::loadModel() to upload textures.\n")
        h.write(f"// Pass bitmaps in the same order as Model_{safe_name}_TexSlot.\n")
        h.write(f"inline void {safe_name}_loadTextures(\n")
        h.write(f"    AnimationController& ctrl,\n")
        h.write(f"    const unsigned int* bitmaps[MODEL_{safe_name.upper()}_TEX_COUNT])\n")
        h.write(f"{{\n")
        if tex_count > 0:
