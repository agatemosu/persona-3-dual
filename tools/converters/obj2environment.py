...
def convert(obj_path, output_dir, config):
    ...
    with open(header_path, "w") as h:
        ...
        h.write(
            f"            displayLists[i] = NULL; dlSizes[i] = 0; textureIDs[i] = 0;\n"
        )
        h.write(f"        }}\n    }}\n\n")
        ...
        if n > 0:
            ...
            h.write(
                f"        if (magic[0]!='E'||magic[1]!='N'||magic[2]!='V'||magic[3]!='1') {{\n"
            )
            ...
            h.write(
                f"            displayLists[i] = (u32*)malloc((dlSizes[i] + 1) * sizeof(u32));\n"
            )
            h.write(f"            displayLists[i][0] = dlSizes[i];\n")
            h.write(f"            if (dlSizes[i] > 0)\n")
            h.write(
                f"                fread(&displayLists[i][1], sizeof(u32), dlSizes[i], file);\n"
            )
            h.write(f"        }}\n")
        ...
        # drawBillboards()
        h.write(
            f"    void drawBillboards(bool faceCamera, float camX, float camY, float camZ) {{\n"
        )
        ...
        h.write(f"                while (GFX_BUSY);\n")
        h.write(
            f"                glBindTexture(GL_TEXTURE_2D, textureIDs[bb.texSlot]);\n"
        )
        h.write(f"                currentSlot = bb.texSlot;\n")
        h.write(f"            }}\n")
        h.write(
            f"            if (!inQuads) {{ glBegin(GL_QUADS); inQuads = true; }}\n\n"
        )
        ...
        h.write(f"            }}\n\n")
        h.write(
            f"            v16 rx = mulf32(rX, bb.halfWidth),  ry = mulf32(rY, bb.halfWidth),  rz = mulf32(rZ, bb.halfWidth);\n"
        )
        h.write(
            f"            v16 ux = mulf32(uX, bb.halfHeight), uy = mulf32(uY, bb.halfHeight), uz = mulf32(uZ, bb.halfHeight);\n\n"
        )
        h.write(
            f"            glTexCoord2t16(bb.u0, bb.v1); glVertex3v16(bb.x-rx-ux, bb.y-ry-uy, bb.z-rz-uz);\n"
        )
        h.write(
            f"            glTexCoord2t16(bb.u1, bb.v1); glVertex3v16(bb.x+rx-ux, bb.y+ry-uy, bb.z+rz-uz);\n"
        )
        h.write(
            f"            glTexCoord2t16(bb.u1, bb.v0); glVertex3v16(bb.x+rx+ux, bb.y+ry+uy, bb.z+rz+uz);\n"
        )
        h.write(
            f"            glTexCoord2t16(bb.u0, bb.v0); glVertex3v16(bb.x-rx+ux, bb.y-ry+uy, bb.z-rz+uz);\n"
        )
        h.write(f"        }}\n")
        ...
        if n > 0:
            ...
            h.write(
                f"            if (displayLists[i]) {{ free(displayLists[i]); displayLists[i] = NULL; }}\n"
            )
            h.write(f"        }}\n")
